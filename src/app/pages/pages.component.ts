...
export class PagesComponent implements OnInit, OnDestroy {
  ...
  styles: object = {
    background: {
      ...
      "background-image":
        "linear-gradient(" +
        this.themes.PUBLIC.PrimaryBackgroundColor +
        "," +
        this.themes.PUBLIC.SecondaryBackgroundColor +
        ")",
    },
    backgroundWidth: 100,
  ...
  ngOnInit() {
    this.sessIns = this.session.session().subscribe((isEnabled) => {
       ...
       * If the page is callback we will navigate to that page itself
      ...
      //navigate to login page if not in callback page and session not loaded
      if (
        !this.sessionEnabled &&
        window.location.href.indexOf("callbacks") == -1
      ) {
      ...
      //we won't do any navigation if the page is a callback
      if (window.location.href.indexOf("callbacks") > -1) {
        return;
      }
